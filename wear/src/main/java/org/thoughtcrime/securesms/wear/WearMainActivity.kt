...
class WearMainActivity : ComponentActivity() {
          ...
          composable(
            ...
            if (threadId != null) {
              ...
              ConversationScreen(
                ...
                onVisible = viewModel::setVisibleThread,
                onHidden = viewModel::clearVisibleThread,
                modifier = Modifier.fillMaxSize()
              )
            }
          }
  ...
  /**
   * WEAR-005 follow-up belt-and-suspenders: clears the reported visible thread on screen-off /
   * app-backgrounded, in case [ConversationScreen]'s `onHidden` didn't already fire (e.g. the
   * Activity is stopped without the composable leaving composition).
   */
  override fun onStop() {
    super.onStop()
